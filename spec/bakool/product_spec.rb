...
describe Bakool::Product do
  ...
  context "without a name or code" do
    it "raises an error" do
      expect { Bakool::Product.new(nil, "P1", 100) }.to raise_error(ArgumentError)
      expect { Bakool::Product.new("Product 1", nil, 100) }.to raise_error(ArgumentError)
    end
  end

  context "with a negative price" do
    it "raises an error" do
      expect { Bakool::Product.new("Product 1", "P1", -100) }.to raise_error(ArgumentError)
    end
  end

  context "without a price" do
    it "defaults to 0" do
      product = Bakool::Product.new("Product 1", "P1")
